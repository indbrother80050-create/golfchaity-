...
export default function Dashboard() {
  ...
  const activateSubMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      if (user) {
        setUser({ ...user, subscription: res.data.data });
      }
    },
  });
  ...
  if (userLoading)
    return (
      <div className="p-12 text-center text-[#6b6b63]">
        Loading dashboard...
      </div>
    );
  if (!user) return null;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 bg-[#f5f5f0] min-h-[calc(100vh-64px)]">
      <div className="mb-12">
        <h1 className="text-5xl font-serif text-[#2c2c28]">
          Hello, {user.name}
        </h1>
        <p className="text-[#6b6b63] mt-3 text-lg">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
            ...
            {scoresLoading ? (
              ...
              <div className="space-y-4">
                {scores.map((score: any) => (
                  <div
                    key={score.id}
                    className="flex items-center justify-between p-5 bg-[#f5f5f0] rounded-2xl border border-[#e8e8e3]"
                  >
                    <div className="flex items-center space-x-5">
                      ...
                      <div>
                        <p className="font-semibold text-[#2c2c28]">
                          Standard Round
                        </p>
              ...
              <div className="text-center py-16 bg-[#f5f5f0] rounded-2xl border border-dashed border-[#d1d1c7]">
                <p className="text-[#6b6b63]">
                  No scores recorded yet. Add your first round!
                </p>
              </div>
            )}
          </section>
        ...
        <div className="space-y-8">
          <section className="bg-[#2c2c28] text-[#f5f5f0] rounded-[32px] p-8 shadow-[0px_4px_20px_rgba(0,0,0,0.05)]">
            ...
            <div className="space-y-5">
              <div className="p-5 bg-white/5 rounded-2xl border border-white/10">
                <p className="text-sm text-[#a3a39b] uppercase tracking-wider mb-1">
                  Current Plan
                </p>
                <p className="text-2xl font-serif">
                  {isActive ? "Pro Monthly" : "No Active Plan"}
                </p>
                {isActive && (
          ...
          <section className="bg-white rounded-[32px] p-8 border border-[#e8e8e3] shadow-[0px_4px_20px_rgba(0,0,0,0.03)]">
            ...
            <p className="text-[#6b6b63] mb-6 leading-relaxed">
              You are currently contributing 10% of your winnings to your
              selected charity.
            </p>
