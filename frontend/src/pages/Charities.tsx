  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-16 bg-[#f5f5f0] min-h-[calc(100vh-64px)]">
      <div className="text-center mb-16">
        <h1 className="text-5xl font-serif text-[#2c2c28] mb-4">
          Our Charity Partners
        </h1>
        <p className="text-[#6b6b63] max-w-2xl mx-auto text-lg leading-relaxed">
          We partner with leading organizations to ensure your contributions
          make a real impact. Choose the cause that matters most to you.
        </p>
      </div>
      ...
      {isLoading ? (
        <div className="text-center py-20 text-[#6b6b63]">
          Loading charities...
        </div>
      ) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {charities?.map((charity: any) => (
            <div
              key={charity.id}
              className="bg-white rounded-[32px] p-8 border border-[#e8e8e3] shadow-[0px_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0px_8px_30px_rgba(0,0,0,0.06)] transition-all flex flex-col group"
            >
              ...
              <h3 className="text-2xl font-serif text-[#2c2c28] mb-3">
                {charity.name}
              </h3>
              <p className="text-[#6b6b63] text-sm mb-8 flex-grow leading-relaxed">
                {charity.description}
              </p>
